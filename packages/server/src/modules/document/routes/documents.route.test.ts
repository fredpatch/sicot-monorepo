...
const getCheminDocument = vi.fn().mockResolvedValue({ chemin: '/tmp/x', nomOriginal: 'x', mimeType: 'text/plain' });
...
describe('documents.route — general library management requires DOCUMENT_* capabilities (agent cannot manage)', () => {
  it.each(NO_LIBRARY_ROLES)('403s role=%s on OCR/category/visibility/delete/restore/reprocess', async (role) => {
    const app = buildApp();
    const cookie = cookieFor(role);
    await request(app).patch('/documents/1/ocr').set('Cookie', cookie).send({ texte: 'x' }).expect(403);
    await request(app).patch('/documents/1/categorie').set('Cookie', cookie).send({ categorie: 'autre' }).expect(403);
    await request(app).patch('/documents/1/visibilite-interne').set('Cookie', cookie).send({ visible: true }).expect(403);
  ...
  it.each(LIBRARY_ROLES)(
    ...
    async (role) => {
      const app = buildApp();
      const cookie = cookieFor(role);
      await request(app).patch('/documents/1/ocr').set('Cookie', cookie).send({ texte: 'x' }).expect(200);
      await request(app).patch('/documents/1/categorie').set('Cookie', cookie).send({ categorie: 'autre' }).expect(200);
      await request(app).patch('/documents/1/visibilite-interne').set('Cookie', cookie).send({ visible: true }).expect(200);
...
describe('documents.route — upload is open to every role (personal-workflow attachment, not library management)', () => {
  ...
  it('an agent cannot self-publish visibiliteInterne=1 through upload — forced to false regardless of the request body', async () => {
    ...
    expect(uploaderDocument).toHaveBeenCalledWith(expect.objectContaining({ visibiliteInterne: false }));
  });
  ...
  it('an operateur CAN set visibiliteInterne=1 through upload', async () => {
    ...
    expect(uploaderDocument).toHaveBeenCalledWith(expect.objectContaining({ visibiliteInterne: true }));
  });
});
...
describe('documents.route — reads scoped by capability, not the literal role "agent"', () => {
  it('agent listing is scoped to own/internal-visible documents (visibleOuUploadePar forced to own id)', async () => {
    ...
    expect(listerDocuments).toHaveBeenCalledWith(expect.objectContaining({ visibleOuUploadePar: 42 }));
  });

  it.each(LIBRARY_ROLES)('role=%s sees the full library (visibleOuUploadePar undefined)', async (role) => {
    const app = buildApp();
    await request(app).get('/documents').set('Cookie', cookieFor(role, 42)).expect(200);
    expect(listerDocuments).toHaveBeenCalledWith(expect.objectContaining({ visibleOuUploadePar: undefined }));
  });

  it('agent aggregates scoped to own id too', async () => {
    const app = buildApp();
    await request(app).get('/documents/aggregates').set('Cookie', cookieFor('agent', 42)).expect(200);
