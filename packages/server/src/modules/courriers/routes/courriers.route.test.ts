...
vi.mock('../controllers/courriers.controller.js', () => ({
  ...
  getFilCorrespondance: (_req: express.Request, res: express.Response) => res.status(200).json({ ok: true }),
  ...
  ajouterDocument: (_req: express.Request, res: express.Response) => res.status(201).json({ ok: true }),
  retirerDocument: (_req: express.Request, res: express.Response) => res.status(200).json({ ok: true }),
}));
...
describe('courriers.route — write routes require CORRESPONDENCE_MANAGE', () => {
  ...
  it.each(NON_MANAGE_ROLES)('403s role=%s on courrier create/update (no CORRESPONDENCE_MANAGE)', async (role) => {
    const app = buildApp();
  ...
  it.each(NON_MANAGE_ROLES)('403s role=%s on document attach/detach (no CORRESPONDENCE_MANAGE)', async (role) => {
    const app = buildApp();
    const cookie = cookieFor(role);
  ...
  it.each(MANAGE_ROLES)('allows role=%s on every write route (has CORRESPONDENCE_MANAGE)', async (role) => {
    const app = buildApp();
    const cookie = cookieFor(role);
