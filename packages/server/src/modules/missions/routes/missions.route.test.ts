...
vi.mock('../controllers/missions.controller', async () => {
  ...
  return {
    ...
    recommandationsEnAttente: (_req: express.Request, res: express.Response) => res.status(200).json({ ok: true }),
    getById: (_req: express.Request, res: express.Response) => res.status(200).json({ ok: true }),
    listerRecommandations: (_req: express.Request, res: express.Response) => res.status(200).json({ ok: true }),
    ...
    ajouterRecommandation: (_req: express.Request, res: express.Response) => res.status(201).json({ ok: true }),
    mettreAJourRecommandation: (_req: express.Request, res: express.Response) => res.status(200).json({ ok: true }),
  };
});
...
describe('missions.route — global write operations require MISSION_MANAGE', () => {
  ...
  it.each(NON_MANAGE_ROLES)('403s role=%s on mission create/update (no MISSION_MANAGE)', async (role) => {
    const app = buildApp();
  ...
  it.each(MANAGE_ROLES)('allows role=%s on mission create/update (has MISSION_MANAGE)', async (role) => {
    const app = buildApp();
    const cookie = cookieFor(role);
...
describe('missions.route — recommendation management requires MISSION_RECOMMENDATION_MANAGE', () => {
  it.each(NON_MANAGE_ROLES)('403s role=%s on recommendation create/update', async (role) => {
    const app = buildApp();
    const cookie = cookieFor(role);
    await request(app).post('/missions/1/recommandations').set('Cookie', cookie).send({}).expect(403);
    await request(app).patch('/missions/recommandations/1').set('Cookie', cookie).send({}).expect(403);
  });

  it.each(MANAGE_ROLES)('allows role=%s on recommendation create/update', async (role) => {
    const app = buildApp();
    const cookie = cookieFor(role);
    await request(app).post('/missions/1/recommandations').set('Cookie', cookie).send({}).expect(201);
    await request(app).patch('/missions/recommandations/1').set('Cookie', cookie).send({}).expect(200);
  });
});

describe('missions.route — read routes unchanged (open to any authenticated role)', () => {
  it.each([...NON_MANAGE_ROLES, ...MANAGE_ROLES])('role=%s can read mission registry', async (role) => {
    const app = buildApp();
    const cookie = cookieFor(role);
    ...
    await request(app).get('/missions/recommandations/en-attente').set('Cookie', cookie).expect(200);
  });
...
describe('missions.route — participant IDOR fix (GET /missions, GET /missions/aggregates)', () => {
  ...
  it('unfiltered global read is unchanged for a non-registry role (no participantId sent to service)', async () => {
    ...
    expect(listerMissions).toHaveBeenCalledWith(expect.objectContaining({ participantId: undefined }));
  });
