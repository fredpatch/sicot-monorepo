...
| `TRUST_PROXY_HOPS`      | `index.ts` (`utils/trustProxy.ts`)                                       | Yes (`packages/server/.env.example`); hardcoded per-environment in `docker-compose.prod.yml`/`docker-compose.staging.yml` (not operator-configurable there, by design - same pattern as `TRANSLATE_SERVICE_URL` below) | Optional, unset/`0` -> trust proxy disabled (default) | No | Number of reverse-proxy hops to trust for `req.ip` (`X-Forwarded-For`/`X-Real-IP`), consumed by every IP-keyed rate limiter (Phase 12.4). Strictly validated: must be an integer, `0`-`10`; any other value is rejected with a console warning and falls back to disabled. Production/staging set `1` (single Nginx hop, per `nginx/prod.conf`/`nginx/staging.conf`); native/Docker dev leaves it unset since no proxy sits in front of the API there - see [../security/csrf-and-session-security.md](../security/csrf-and-session-security.md#rate-limiting). |
...
| `BACKUP_NAS_DIR`   | `jobs/backup.ts`                                              | Yes      | Optional, default `/mnt/nas/sicot/backups`; set to an empty string to disable NAS replication explicitly | No        | Deliberately **not** DB-configurable - IT-managed mount, env-only. Replication target only (the completed local set is copied here). The job never creates this root; if the resolved path is unmounted/absent, replication reports `indisponible`; if explicitly empty, `non_configure`. |
...
`scripts/restore-backup.mjs` (Phase 12.3B) takes no environment variables
at all - `--backup-set`, `--database-url`, and `--upload-dir` are always
explicit CLI arguments, deliberately with no fallback to `DATABASE_URL` /
`UPLOAD_DIR`, so a restore invocation can never silently inherit whatever
this process happens to be configured for. See
